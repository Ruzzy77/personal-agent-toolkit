...
description: Reflect confirmed Journal outcomes before or after week close into the relevant existing Corpus project context or indexed Work source without creating an aggregate Journal archive.
---

# Journal 결과를 Corpus에 반영

`journal_prepare_week_close`에서 `corpusCandidates`가 반환되었거나 사용자가 확정된 Journal 결과를 프로젝트 맥락에 반영해 달라고 할 때 적용합니다. 마감 주는 `journal_get_board`의 `closure.corpusCandidates`에서 고정된 후보와 현재 `reflectionStatus`를, `closure.corrections`에서 후속 정정을 읽습니다. 마감 준비를 다시 만들거나 주를 다시 열지 않습니다.

1. 후보와 관련 정정, 필요한 항목 이력을 함께 읽습니다. 정정으로 원래 결과가 무효가 됐다면 그 후보를 그대로 적용하지 않습니다. 정정된 내용은 해당 프로젝트 정본에서 처리하고, 기존 후보를 반영하지 않기로 결정했다면 정정 사건 ID와 이유를 `details`에 남겨 `skipped`로 기록합니다. 이미 완료된 영수증은 바꾸지 않으며 후속 개정의 근거는 correction과 프로젝트 정본에 남깁니다. 후보의 `targetSpace`, 프로젝트와 `durableOutcome`을 확인합니다. 대상이 없거나 임시 활동 기록이면 Journal에만 남깁니다.
...
6. 주간 마감도 요청받았다면 `manage-journal`에 따라 현재 `preparationVersion`으로 확정합니다. 미처리·실패 후보는 마감을 막지 않으며 마감 후에도 같은 후보 식별자와 hash로 영수증을 추가할 수 있습니다. 마감 자체를 반영 완료나 건너뛰기 결정으로 해석하지 않습니다. 결과 반영 요청만으로 주간 마감까지 승인됐다고 보지 않습니다.
