...
For repeated `corpus_space_get` or `corpus_document_read` freshness checks, use `if_none_match` only when the complete earlier representation is still available in the current context. Set it to `null` on the first selected representation, then reuse the returned `read_etag` only with the same operation and selection. `not_modified=true` refers to that exact representation; after compaction or when only a summary or tag remains, fetch the full representation again. Do not revalidate merely to demonstrate currency when the task does not require it. This conditional form does not apply to Source or Work file reads.
