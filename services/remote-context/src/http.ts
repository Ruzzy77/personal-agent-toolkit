...
async function upsertSyncConnections(
  ...
  if (!registered) throw new ContextError("device_not_registered", "Connect the registered Sync device before publishing Connections", 409);
  const keys = input.connections.map((row) => `${row.spaceId}:${row.connectionId}`);
  if (new Set(keys).size !== keys.length || Object.keys(input.expected_generations).length !== keys.length ||
      keys.some((key) => !Object.hasOwn(input.expected_generations, key))) {
    throw new ContextError("invalid_connection_batch", "Each unique Connection requires exactly one expected generation");
  }
  ...
  for (const row of input.connections) {
    ...
    if (row.deviceId !== deviceId) throw new ContextError("connection_device_mismatch", "Connection must belong to the authenticated Sync device", 403);
    if (row.localConnectionKey !== null && !/^[A-Za-z0-9][A-Za-z0-9._:@-]{0,499}$/.test(row.localConnectionKey)) {
      throw new ContextError("private_path_rejected", "Local Connection keys must be opaque identifiers, not filesystem paths");
    }
    if (new Set(row.roles).size !== row.roles.length ||
        (row.roles.includes("source") && !row.corpusId) ||
        (row.indexMode === "indexed" && (!row.roles.includes("source") || !row.corpusId)) ||
        (row.permission !== "read_only" && !row.roles.includes("work"))) {
      throw new ContextError("invalid_connection_policy", "Connection roles, indexing, and permission do not agree");
    }
    const space = await env.STATE_DB.prepare("SELECT 1 FROM corpus_spaces WHERE owner_id=? AND space_id=? AND state='active'")
      .bind(ownerId, row.spaceId).first();
    if (!space) throw new ContextError("space_not_found", "Connection requires an existing active Space", 404);
    const current = await env.STATE_DB.prepare(`SELECT roles_json,access_scope,permission,index_mode,corpus_id,device_id,local_connection_key,generation
      FROM corpus_connections WHERE owner_id=? AND space_id=? AND connection_id=?`)
      .bind(ownerId, row.spaceId, row.connectionId).first<{
        roles_json: string; access_scope: string; permission: string; index_mode: string;
        corpus_id: string | null; device_id: string | null; local_connection_key: string | null; generation: number;
      }>();
    if (current?.device_id && current.device_id !== deviceId) {
      throw new ContextError("connection_device_mismatch", "Another Sync device owns this Connection", 403);
    }
    if ((expected === "absent" && current) || (expected !== "absent" && current?.generation !== expected)) {
      throw new ContextError("connection_generation_conflict", "Connection generation changed or its absence was not confirmed", 409, { connection_key: key });
    }
    const rolesJson = canonicalJson([...row.roles].sort());
    const authorityChanged = current && (
      canonicalJson((JSON.parse(current.roles_json) as string[]).sort()) !== rolesJson ||
      current.access_scope !== row.accessScope || current.permission !== row.permission ||
      current.index_mode !== row.indexMode || current.corpus_id !== row.corpusId ||
      current.device_id !== row.deviceId || current.local_connection_key !== row.localConnectionKey
    );
    if (!Number.isSafeInteger(row.generation) || (current && (row.generation < current.generation ||
        (authorityChanged && row.generation <= current.generation)))) {
      throw new ContextError("connection_generation_required", "Authority changes require a newer Connection generation", 409, { connection_key: key });
    }
    ...
    const identityGuard = expected === "absent"
    ...
    guards.push(env.STATE_DB.prepare(`SELECT CASE WHEN
      EXISTS (SELECT 1 FROM sync_devices WHERE owner_id=? AND device_id=? AND status='active') AND
      EXISTS (SELECT 1 FROM corpus_spaces WHERE owner_id=? AND space_id=? AND state='active') AND
      ${identityGuard} THEN 1 ELSE json('sync_connection_conflict') END AS valid`)
      .bind(ownerId, deviceId, ownerId, row.spaceId, ownerId, row.spaceId, row.connectionId,
        ...(expected === "absent" ? [] : [expected, deviceId])));
    writes.push(env.STATE_DB.prepare(`INSERT INTO corpus_connections(owner_id,space_id,connection_id,display_name,roles_json,access_scope,
      ...
      .bind(ownerId, row.spaceId, row.connectionId, row.displayName, rolesJson, row.accessScope, row.permission, row.indexMode,
        row.corpusId, deviceId, row.localConnectionKey, row.generation, row.configurationState, row.sourceState, row.recordState, row.capturedAt, now));
  }
  try {
    await env.STATE_DB.batch([...guards, ...writes]);
  } catch (error) {
    if (error instanceof Error && error.message.includes("malformed JSON")) {
      throw new ContextError("connection_generation_conflict", "Connection, Space, or device changed before the batch; no Connections were updated", 409);
    }
    throw error;
  }
  return { updated_connections: input.connections.map((row) => ({ space_id: row.spaceId, connection_id: row.connectionId, generation: row.generation })),
    updated_count: input.connections.length, updated_at: now };
}
...
async function syncRoutes(
  ...
  if (request.method === "POST" && url.pathname === "/sync/v1/connections:upsert") {
    return json({ ok: true, result: await upsertSyncConnections(env, principal, await readJson(request)) });
  }
