...
항목에는 짧은 제목, 현재 상태, 프로젝트 키, 출처 종류와 참조만 둡니다. 이메일 본문, 파일 내용, 대화 원문, 사용자 자격 증명은 저장하지 않습니다. Corpus 반영 후보는 `durable_outcome`과 `corpus_target_space`가 명시된 항목만 대상으로 하며, Journal의 자동 요약을 곧바로 Corpus 색인에 넣지 않습니다.
