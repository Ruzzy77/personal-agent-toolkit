...
- 현재 주 보드는 이전 주의 최신 `active`·`held` 항목을 읽기 전용으로 합쳐 보여 줍니다. 쓰기 시 현재 주 인스턴스를 원자적으로 만들며 과거 기록과 보류·대기 상태를 보존합니다. 명시적인 마감도 보류 상태를 유지합니다.
...
- 주간 마감은 준비와 확정을 분리합니다. 준비 버전 이후 항목이 바뀌면 확정을 거부하고, 미처리·실패 Corpus 후보가 있어도 소유자의 명시적 요청으로 주를 닫습니다. 후보는 고정하고 반영 영수증과 정정은 마감 뒤에도 추가합니다. 마감 주 보드는 고정 후보와 현재 반영 상태·정정을 함께 반환합니다.
