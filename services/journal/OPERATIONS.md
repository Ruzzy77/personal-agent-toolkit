...
스크립트는 새 파일과 상위 폴더를 처음 만들 때부터 개인 전용 권한을 사용합니다. 기존 백업과 다른
경로를 사용하고, 완료 뒤 파일 권한이 `600`인지와 파일이 비어 있지 않은지 확인합니다. 백업에는
항목 제목과 요약이 포함되므로 동기화 공유 폴더나 저장소에 두지 않습니다.
...
2026-09-05에는 운영 백업을 새 로컬 저장소에 복원하여 8개 테이블의 스키마·모든 행 값·건수와
무결성·외래 키를 대조했습니다. 기존 HTTP API로 열린 주와 항목 21개를 읽었고, 항목 상세의
`corpus_promoted` 이력이 복원 영수증의 대상·경로·내용 hash·상태와 일치했습니다. 영수증 전체
레코드를 반환하는 GET은 없으므로 이력의 필드 대조와 구분합니다. 백업에 이월 사건·마감 주·
기간 요약이 없어 해당 대표 읽기는 확인하지 않았습니다. 운영 쓰기 없이 임시 파일과 프로세스를
정리했으며, 실제 복구에서도 없는 표본을 검증 완료로 처리하지 않습니다.
...
- **Corpus 반영 실패**: 실패 원인과 `failed` 영수증을 보존합니다. 명시적으로 요청한 마감은 허용하되 미반영을 완료로 처리하지 않습니다. 마감 후에는 보드의 고정 후보·반영 상태와 correction을 읽고 이어갑니다. 같은 요청의
  전송 결과만 불확실하면 같은 멱등 키를 사용합니다. 실패 뒤 새 시도에는 새 키를 사용하되 Corpus의
  실제 저장 상태를 먼저 다시 읽어 중복 적용을 피합니다. 실패 원인이 해소되지 않거나 후보가 바뀌면
  재시도를 멈춥니다. 실패 영수증이나 반영 요청 자체는 마감 허가가 아니며 완료된 후보를 새 실패로 되돌리지 않습니다.
