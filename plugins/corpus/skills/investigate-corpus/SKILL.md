...
For a body-only revision, carry forward the current `kind`, `applicability`, `source_refs` and `migration_provenance` explicitly; omission is not a preservation contract. Keep only the reference fields accepted by the write schema, and supply the required explicit approval when revising guidance. On conflict, reread and reconcile the content and metadata rather than changing only the expected version.
