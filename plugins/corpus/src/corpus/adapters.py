...
ENVELOPE_SCHEMA_VERSION = "corpus.extraction-envelope.v2"
...
class CoverageProfile:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, object]) -> CoverageProfile:
        if set(value) != _COVERAGE_DIMENSIONS:
            raise ExtractionError(
                "external adapter coverage dimensions are invalid",
                details={"dimensions": sorted(value)},
            )
        return cls(**{name: value[name] for name in _COVERAGE_DIMENSIONS})
...
def _issue_from_mapping(
    ...
    *,
    ...
    if strict:
        required.update({"impact", "coverage_dimensions"})
    keys = set(raw)
...
class ExternalJSONLAdapter:
    ...
    def _validate_result(self, raw: Mapping[str, object]) -> ExtractionEnvelope:
        _strict_fields(
            ...
            required={"schema_version", "completeness", "coverage", "units"},
            location="$",
        )
        ...
        if not isinstance(raw_coverage, Mapping):
            raise ExtractionError("external adapter coverage must be an object")
        ...
        declared_coverage = CoverageProfile.from_mapping(raw_coverage)
