...
같은 작업에서는 새 입력이 보이지 않아 Source 호출 전에 중단했습니다. 이 결과를 원격 서버의
구형 배포나 인증 장애로 단정하지 않았습니다. 이후 사용자가 허용한 새 Cowork 작업에서 현행
입력과 저장된 출처 한 개의 읽기를 확인했습니다. document·revision·projection·unit 식별자가
일치했고 원 Source의 unavailable·stale_source_revision 표시를 현재 자료의 확보로 해석하지
않았습니다. 같은 작업이 별도로 공통 지침의 빈 Context를 본문 접근 불가로 판단한 경우에는
현재 Work 연결과 파일 읽기 경로를 지정한 뒤 실제 파일을 읽었습니다. Source와 Work 모두
등록·갱신·쓰기 없이 확인했으며, 안내 후 성공과 자동 조회 경로 선택은 구분합니다.
