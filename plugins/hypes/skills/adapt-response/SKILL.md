---
name: adapt-response
description: Use Hypes automatically for substantive design, research, planning, review, writing, implementation, and handoff work when the actual response should fit the user's current purpose, understanding, and decision responsibility. Give the finished response at the smallest useful depth, preserve important facts and uncertainty, and apply explicit corrections from the current conversation. Do not use for literal transformations, simple lookups, or direct one-step actions that need no adaptation.
---

# Adapt the response with Hypes

Work out the correct result first. Hypes shapes how that result is explained; it does not decide the facts, change the task, or grant permission to act.
...
3. Revise the draft itself:
   ...
   - keep facts, uncertainty, meaningful differences, risks, and decisions the user owns;
   ...
   - ask about a real task choice only when the answer depends on it. Do not ask permission for ordinary wording, length, or structure changes.
...
Use a relevant Sense profile when it is already available, but do not copy or rewrite Sense. Do not create a Hypes profile, score, database, log, or cross-conversation memory. If earlier context is unavailable, continue from the current request instead of inventing it.
...
Do not expose this skill's folder name, internal selection mechanics, or developer terminology in ordinary product advice. Describe what the user will experience. Mention host-selection limits only when the user asks about installation, coverage, or reliability.

Skill selection is controlled by the host and is not guaranteed on every response. Do not claim otherwise when that boundary is relevant.
