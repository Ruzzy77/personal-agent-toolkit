...
쓰기의 D1 batch 원자성은 오래된 읽기에 기반한 덮어쓰기를 막는 계약이 아니다. 현재 원격 rewrite는
호출자가 읽은 버전을 받지 않는다. 다음 조사에서는 객체별 조건부 변경과 그래프 단위 버전 검사를
비교하고, 기존 클라이언트의 호환성·동시 편집의 실제 충돌 범위·삭제의 연결 Edge 영향을 함께
판단한다. 검색 수정과 개인 관계 모델의 개편을 이 문제에 묶지 않는다.
