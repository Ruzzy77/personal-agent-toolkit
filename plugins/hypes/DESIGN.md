...
관계 조회는 설명의 시작점, 요청의 해석이나 선택을 실제로 바꿀 때 사용한다. 현재 대화와 Sense로
그 판단이 정해졌다면 같은 내용을 확인하기 위해 Hypes를 다시 읽지 않는다. 필요한 관계와 적용
범위를 함께 읽고, 범위가 한정된 관계를 사용자의 일반적 성향이나 고정된 지식 수준으로 확대하지 않는다.

2026-09-05 검토에서는 현재의 관계 저장과 표적 조회를 유지하기로 했다. 한글 질의의 관계·적용 범위
반환은 확인했지만, 그래프가 단순한 메모보다 상호작용의 품질을 높인다는 비교 근거는 얻지 못했다.
이를 입증하려고 운영 관계를 재작성하거나 별도 평가 시스템을 만들지 않는다. 현재 구조를 다른
저장 형식으로 이전하는 비용을 추가하지 않고, 새 추론 서비스·임베딩 검색·상시 전체 조회도 도입하지 않는다.

후속 사용에서 필요한 관계를 반복해서 찾지 못하거나 여러 단계의 연결이 실제 설명·선택에 도움이
되는 사례가 생기면 그 경로만 다시 검토한다. 현재 검색과 별칭으로 해결할 수 있는 문제인지 먼저
구별하며, 사용자 모델의 역할이나 개인 관계의 일괄 개편은 검색 개선과 분리한다.
