...
주간 마감에서 `corpusCandidates`가 반환되었거나 사용자가 확정된 Journal 결과를 프로젝트 맥락에 반영해 달라고 할 때만 적용합니다.
...
5. 저장과 재색인을 다시 읽어 확인한 뒤 `journal_record_corpus_promotion`으로 대상 Space, 실제 source path, content hash와 적용 상태를 기록합니다. 이미 같은 결과가 반영된 receipt가 있으면 중복 작성하지 않습니다.
