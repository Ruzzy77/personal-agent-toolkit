...
새로고침 후 보관 초안 표시, 복원 후 검토 상태 유지와 제안 취소가 동작했다. 점검 전후 같은 호의
전체 HTML·참고자료·표지 경로·발행 정보·갱신 시각과 version이 동일했다. 직접 입력의 자동 저장과
운영 자료를 변경하는 충돌 시험은 하지 않았다.

390px 화면에서 가로 넘침 없이 초안 안내·복원·복사 버튼과 본문을 읽었고, 키보드로 이동한 복사
버튼의 포커스가 보였다. 글자 크게 기능으로 본문 크기가 증가한 뒤 원래 크기로 돌아오는 것도
확인했다. 이는 전체 접근성 감사나 200% 확대 검증이 아니다. 확인용 보관 초안은 후속 새로고침에서도
더 이상 표시되지 않음을 확인했고, 기존 사용자 기록은 정리 대상이 아니다. 운영 코드 변경이나
재배포는 하지 않았다.
...
발간 분류·날짜와 삭제 상태를 분리한다. 발간호 휴지통·복원·영구 삭제는 검토한 버전과 영향 토큰을 검사하며 대화와 관리 화면에 함께 제공한다. 새 삭제 묶음은 최초 서버 저장 시각부터 30×24시간 보존하고, 자동 정리가 켜져 있으면 한국시간 오전 4시 정리 주기에 처리한다. 수동 영구 삭제에는 소유자의 명시적 확인이 필요하다.

표지와 지원되는 본문 `/media/` 참조를 제품 색인에 보관한다. 공통 불변 객체 모듈은 같은 바이트·MIME의 재사용만 맡고 소유·참조·삭제 상태는 Library가 판단한다. 공유 객체를 제외하고 신규 참조를 DB에서 차단한 뒤 R2를 삭제한다. R2 결과를 확인하지 못하면 처리 상태를 남겨 재개한다. 기존 미참조 객체와 소유 관계가 불명확한 객체를 일괄 정리하지 않는다. 자동 저장 경험은 유지한다. [공통 관리 계약](../../services/remote-context/MANAGEMENT.md)을 함께 적용한다.
