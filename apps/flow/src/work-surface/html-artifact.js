...
export function htmlArtifactDocument(html,assets,channel,theme='light'){
 ...
 script.textContent='(()=>{const channel='+json(channel)+';let previous=0;const size=()=>{const height=Math.ceil(Math.max(document.body.scrollHeight,document.body.getBoundingClientRect().height));if(height!==previous){previous=height;parent.postMessage({type:"flow-artifact-size",channel,height},"*")}};new ResizeObserver(size).observe(document.body);addEventListener("load",size);addEventListener("message",e=>{if(e.source===parent&&e.data?.channel===channel&&["light","dark"].includes(e.data.theme)){document.documentElement.dataset.theme=e.data.theme;size()}});size()})();';
...
export function htmlArtifactContainer(document,channel){
 ...
 const script='(()=>{const channel='+json(channel)+';const frame=document.querySelector("iframe");frame.srcdoc='+json(document)+';addEventListener("message",e=>{if(e.data?.channel!==channel)return;if(e.source===frame.contentWindow&&e.data.type==="flow-artifact-size"&&Number.isFinite(e.data.height))parent.postMessage({type:e.data.type,channel,height:e.data.height},"*");else if(e.source===parent&&["light","dark"].includes(e.data.theme))frame.contentWindow.postMessage({channel,theme:e.data.theme},"*")})})();';
