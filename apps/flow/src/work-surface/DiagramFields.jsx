...
export function DiagramFields({content,onChange,busy=false,headingLabel="소제목",headingMaxLength=500}) {
    ...
    return <div className="flow-content-fields">
      ...
      <div className="flow-content-list-actions">
        <Button type="button" color="primary" variant="ghost" pill={false} size="sm" disabled={busy||nodes.length>=100} onClick={addNode}>항목 추가</Button>
        <Button type="button" color="primary" variant="ghost" pill={false} size="sm" disabled={busy||nodes.length<2} onClick={()=>onChange({...content,nodes:layoutDiagramNodes(nodes,"vertical")})}>세로 배치</Button>
        <Button type="button" color="primary" variant="ghost" pill={false} size="sm" disabled={busy||nodes.length<2} onClick={()=>onChange({...content,nodes:layoutDiagramNodes(nodes,"horizontal")})}>가로 배치</Button>
      </div>
      {nodes.map((node,index)=><div className="flow-content-item" key={node.id}>
        ...
        <Button type="button" color="primary" variant="ghost" pill={false} size="sm" disabled={busy||nodes.length<=1} onClick={()=>removeNode(node.id)}>항목 삭제</Button>
      </div>)}
      <div className="flow-content-list-actions">
        <span className="flow-content-item-heading">연결</span>
        <Button type="button" color="primary" variant="ghost" pill={false} size="sm" disabled={busy||!next||edges.length>=200} onClick={()=>onChange({...content,edges:[...edges,{id:crypto.randomUUID(),...next,label:""}]})}>연결 추가</Button>
      </div>
      {edges.map((edge,index)=><div className="flow-content-item" key={edge.id}>
        ...
        <Button type="button" color="primary" variant="ghost" pill={false} size="sm" disabled={busy} onClick={()=>{onChange({...content,edges:edges.filter(item=>item.id!==edge.id)});if(selected?.kind==="edge"&&selected.id===edge.id)setSelected(null)}}>연결 삭제</Button>
      </div>)}
    </div>;
}
