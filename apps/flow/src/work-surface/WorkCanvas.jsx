...
export function WorkCanvas({work,artifactId,onSelect,renderers,prepareContent,resolveMediaUrl}){
 ...
 const headingKey=artifact.id+':'+artifact.revision;
 const split=artifact.kind==='content'?splitContentHeading(artifact.composition):null;
 const hasHeading=(artifact.kind==='html'&&(htmlHeading?.key===headingKey?htmlHeading.value:true))||Boolean(split?.heading)||(artifact.kind==='document'&&artifact.blocks?.[0]?.heading?.trim()===artifact.title?.trim());
