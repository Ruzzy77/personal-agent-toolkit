...
def test_http_auth_ranges_and_safe_preview(tmp_path):
    ...
    headers = {"Authorization": f"Bearer {UPSTREAM}", "X-Toolkit-Transfer-Token": receipt["token"]}
    assert client.get(path).status_code == 401
    assert client.get(path, headers={"Authorization": f"Bearer {UPSTREAM}"}).status_code == 404
    ...
    assert client.get(path, headers={**headers, "Range": "bytes=999-"}).status_code == 416
...
def test_upload_chunk_http_limit_and_commit(tmp_path):
    ...
    client = TestClient(BearerGuard(Starlette(routes=TransferHTTP(transfers).routes()), UPSTREAM))
    path = f"/transfers/{receipt['transfer_id']}"
    headers = {
        "Authorization": f"Bearer {UPSTREAM}", "X-Toolkit-Transfer-Token": receipt["token"],
        "Content-Type": "application/octet-stream", "Upload-Offset": "0",
    }
    assert client.put(path + "/chunk", content=b"x" * (CHUNK_BYTES + 1), headers=headers).status_code == 413
    assert not (root.root / "binary.bin").exists()
    assert client.put(path + "/chunk", content=b"abc", headers=headers).status_code == 200
...
def test_html_cannot_navigate_run_code_or_load_external_assets():
    ...
    for forbidden in ["http-equiv", "<script", "<iframe", "<form", "onclick", "href=", "outside.test"]:
