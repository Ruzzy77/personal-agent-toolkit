...
현재 보드는 이전 주의 미완료·보류를 한 목록에서 이어 보여 줍니다. 별도 주간 마감은 필요하지 않으며 모든 미완료가 표시됩니다. 상태 변경 후 새 주간 인스턴스가 반환되면 새 ID로 다시 읽습니다.
...
상단 배치, 바깥 여백과 공통 색상·간격은 저장소에 포함한 UI Kit 1.2.0을 사용합니다. 기록판의
상태 의미와 Journal 고유 업무 흐름은 이 Site에 유지하며 공통 패키지 안으로 옮기지 않습니다.
