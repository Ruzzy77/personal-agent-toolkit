...
const recipeIds = availableRecipes.map((recipe) => recipe.id);
...
function mergeLabels(pick: (gallery: Gallery) => Record<string, string> | undefined) {
...
const templateLabels = mergeLabels((gallery) => gallery.template_labels);
const useLabels = mergeLabels((gallery) => gallery.use_labels);
const avoidLabels = mergeLabels((gallery) => gallery.avoid_labels);
...
function patternNames(recipe: Recipe) {
  return recipe.pattern_refs.map((id) => patternById.get(id)?.name || id);
}
...
function referenceReasons(
  ...
  const names = patternNames(recipe);
...
function rankReferences(
  ...
  return availableRecipes
    .map((recipe, index) => ({
      ...
      reasons: referenceReasons(recipe, format, content, selectedNeeds),
...
function noteFor(recipe: Recipe): RecipeNote {
  ...
  const purpose = labelsFor(recipe.use_for, useLabels)[0] || "등록된 레시피";
  return {
...
function buildRequestText({
  ...
  const requirementLine = selectedNeeds.length
    ...
    : "";
  const patternLine = patternNames(recipe).join(", ");
  ...
  return [
    ...
    `참고 방향: ${noteFor(recipe).koreanName} 레시피${patternLine ? ` · ${patternLine}` : ""}`,
    `${noteFor(recipe).koreanName} 레시피를 참고해 ${requestFormatName(format)} 결과물을 만들어 주세요. 레시피를 그대로 복제하지 말고 목적에 맞는 조형 원리와 재료만 선택해 적용해 주세요. 기존 브랜드나 디자인 시스템이 있으면 유지하고, 충돌하는 색상·서체·구성은 바꾸지 마세요.`,
    requirementLine,
    improvementLine,
  ].filter(Boolean).join("\n\n");
}
...
function parseReferenceId(value: unknown) {
  if (typeof value !== "string" || !recipeIds.includes(value)) {
...
function asCompareInput(input: unknown) {
  ...
  const referenceIds = value.reference_ids.map(parseReferenceId);
...
function asBriefInput(input: unknown) {
  const value = assertObject(input, ["reference_id", "format", "content", "subject", "needs"]);
  const referenceId = parseReferenceId(value.reference_id);
  const format = parseFormat(value.format);
  const content = parseContent(value.content);
...
function referencePayload(reference: RankedReference, rank: number) {
  return {
    rank,
    reference_id: reference.recipe.id,
    name: noteFor(reference.recipe).koreanName,
    version: reference.recipe.version,
    patterns: reference.recipe.pattern_refs.map((id) => ({
      id,
      name: patternById.get(id)?.name || id,
    })),
    ...
    >
      ...
      <div className="card-body">
        ...
        <ul className="pattern-tags" aria-label={`${note.koreanName} 설계 패턴`}>
          {patternNames(recipe).map((name) => <li key={name}>{name}</li>)}
        </ul>
        ...
        {showTemplates ? (
          <div className="template-links" aria-label={`${note.koreanName} HTML 틀`}>
            ...
            {Object.entries(recipe.templates).map(([templateName, templatePath]) => (
              <a key={templateName} href={`/templates/${recipe.id}/${templatePath}`} target="_blank" rel="noreferrer">
                {templateLabels[templateName] || templateName}
              </a>
            ))}
          </div>
        ) : null}
...
function CompareColumn({ recipe, format }: { recipe: Recipe; format: FormatId }) {
  const note = noteFor(recipe);
  return (
    <section className={`compare-column compare-column--${recipe.id}`}>
      ...
      <div className="compare-group">
        <h4>연결 패턴</h4>
        <ul>{patternNames(recipe).map((label) => <li key={label}>{label}</li>)}</ul>
      </div>
      ...
      <div className="compare-group">
        <h4>잘 맞는 내용</h4>
        <ul>{labelsFor(recipe.use_for, useLabels).map((label) => <li key={label}>{label}</li>)}</ul>
      </div>
      ...
      <div className="compare-group compare-group--avoid">
        <h4>잘 맞지 않는 내용</h4>
        <ul>{labelsFor(recipe.avoid_for, avoidLabels).map((label) => <li key={label}>{label}</li>)}</ul>
      </div>
    </section>
  );
}
...
function RequestBuilder({
  ...
  const requestText = useMemo(
    () => buildRequestText({ recipe, format, content, subject, selectedNeeds, selectedDirections }),
      ...
      <div className="request-grid">
        ...
        <div className="request-output">
          ...
          <div className="request-actions">
            ...
            {showTemplate ? (
              <a href={`/templates/${recipe.id}/${firstTemplate[1]}`} target="_blank" rel="noreferrer">
                {templateLabels[firstTemplate[0]] || firstTemplate[0]} HTML 틀
              </a>
            ) : null}
          </div>
          {copyState !== "idle" ? (
...
export default function Home() {
  ...
  const rankedReferences = useMemo(
    () => rankReferences(format, content, selectedNeeds, referenceLimit),
    [content, format, referenceLimit, selectedNeeds],
  );
  ...
  useEffect(() => {
    ...
    const tools: WebMcpTool[] = [
        ...
        async execute(input) {
          ...
          return {
            ...
            references: references.map((reference, index) =>
              referencePayload(reference, index + 1),
            ),
          };
        },
      },
      {
        ...
        annotations: commonAnnotations,
        async execute(input) {
          const value = asCompareInput(input);
          if (value.format) {
            setFormat(value.format);
      ...
      },
      {
        ...
        annotations: commonAnnotations,
        async execute(input) {
          const value = asBriefInput(input);
          ...
          <aside className="finder-result" aria-live="polite">
            ...
            <div className="candidate-list">
              {rankedReferences.map((reference, index) => {
                const note = noteFor(reference.recipe);
                return (
      ...
      {comparedIds.length > 0 && (
        <aside className="compare-tray" aria-label="비교할 참고 후보">
          <div>
            ...
            <p>{comparedRecipes.map((recipe) => noteFor(recipe).koreanName).join(" · ")}</p>
          </div>
      ...
      >
        {previewRecipe ? (
          <section className="preview-panel">
            <header>
              <h2 id="preview-title">{noteFor(previewRecipe).koreanName} 미리보기</h2>
              <div className="panel-actions">
                <a
                  href={`/previews/${previewRecipe.id}/index.html`}
                  target="_blank"
                  rel="noreferrer"
                >
                  새 탭에서 보기
            ...
            <iframe
              src={`/previews/${previewRecipe.id}/index.html`}
              title={`${noteFor(previewRecipe).koreanName} 디자인 미리보기`}
            />
          </section>
        ) : null}
      </Modal>
          ...
          <section className="compare-panel">
            ...
            <div className="compare-grid">
              {comparedRecipes.map((recipe) => (
                <CompareColumn key={recipe.id} recipe={recipe} format={format} />
              ))}
            </div>
          </section>
        ) : null}
