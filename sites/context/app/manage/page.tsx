...
import { contextCall, ContextFailure } from "../../lib/context";
...
export default function ManagementPage() {
  ...
  const load = useCallback(async (spaceId: string) => {
    ...
    const chosen =
      found.find((s) => s.space_id === spaceId) ??
      found.find((s) => !s.deletion_group_id);
    if (!chosen) {
      setSelected("");
      setDetail({});
      return;
    ...
    setRegistrations(
      await contextCall("corpus_registrations_list", { space_id: id }),
    );
  }, []);
  useEffect(() => {
    let active = true;
  ...
  async function applyImpact() {
    ...
    await work(async () => {
      ...
      if (impact.action === "trash") {
        ...
        const version = impact.members.find(
          (m) => m.kind === target.kind,
        )!.version;
        result = await contextCall(`corpus_${target.kind}_trash`, {
          ...
          ...(target.kind === "document"
            ...
            : target.kind === "context_item"
              ...
              : {}),
          expected_version:
            target.kind === "context_skill" ? version : Number(version),
        });
      } else {
  ...
  return (
    <main className="management-page">
      <header>
        <Link href="/">← 읽기·편집으로</Link>
        <Link href="/manage/sense">Sense 관리</Link>
        <h1>자료 관리</h1>
        <p>
          자료를 이동하거나 휴지통에서 복원합니다. 원본 파일은 바뀌지 않습니다.
        </p>
      </header>
      ...
      <section aria-labelledby="space-heading">
        ...
        <div className="management-actions">
          ...
          <button
            disabled={busy}
            onClick={() => void work(() => load(selected))}
          >
            새로고침
          </button>
        </div>
                ...
                onClick={() =>
                  void work(async () => {
                    await contextCall("corpus_space_revise", {
                      ...
                      state:
                        detail.state === "archived" ? "active" : "archived",
                    });
                    await load(selected);
          ...
          {targets.map(({ target, title }) => (
            <li key={target.kind + ":" + target.id}>
              ...
              <div className="management-actions">
                ...
                <button
                  disabled={busy}
                  onClick={() => void review("trash", target)}
                >
                  휴지통으로
                </button>
              </div>
        ...
        <details>
          ...
          >
            ...
            <button disabled={busy || (!selected && createKind !== "space")}>
              새로 저장
            </button>
          </form>
        </details>
      ...
      {selected && (
        <section>
          <h2>등록 연결</h2>
          <p>
            Sync가 실행 중인 작업의 종료와 로컬 연결 해제를 확인해야 완료됩니다.
            원본 파일과 보존된 Source는 삭제하지 않습니다.
          </p>
          <ul className="management-list">
            {rows(registrations.connections).map((connection) => (
              <li key={String(connection.connection_id)}>
                <div>
                  ...
                  {String(connection.configuration_state)} · 버전{" "}
                  {String(connection.generation)}
                </div>
                <button
            ...
            {rows(registrations.workspaces).map((workspace) => (
              <li
                key={String(workspace.host_id) + String(workspace.workspace_id)}
              >
                <div>
            ...
            <p key={String(row.registration_key)}>
              ...
              {row.state === "detached"
                ...
                : row.state === "blocked"
                  ? "확인 필요"
                  : "Sync 확인 대기"}
              {record(row.result).reason
                ? ` · ${String(record(row.result).reason)}`
                : ""}
            </p>
          ))}
          {detachTarget && (
      ...
      <section aria-labelledby="trash-heading">
        ...
        <p>
          삭제한 시각부터 30일이 지나면 다음 정리 때 영구 삭제합니다. 참조나
          연결이 남은 자료는 보존합니다.
        </p>
        <p>
          ...
          {trash.map((group) => (
            <li key={String(group.deletion_group_id)}>
              <div>
                ...
                <p>
                  {date(group.trashed_at)} 삭제 · {date(group.purge_after)} 이후
                  정리
                </p>
                <details>
                ...
                <button
                  ...
                  onClick={() =>
                    void review(
                      "restore",
                      undefined,
                      String(group.deletion_group_id),
                    )
                  }
                >
                  묶음 복원
      ...
      {impact && (
        ...
        >
          ...
          {impact.action === "trash" && (
            <p>
              30일 뒤 영구 삭제 대상이 됩니다. 원본 파일이나 연결 설정은
              삭제하지 않습니다.
            </p>
          )}
          ...
          <div className="management-actions">
            <button
              disabled={
                busy ||
                (impact.action === "purge" &&
                  (!confirmed || Boolean(impact.blockers.length)))
              }
