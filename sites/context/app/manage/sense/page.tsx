...
import { contextCall } from "../../../lib/context";
...
export default function SenseManagementPage() {
  ...
  const load = useCallback(async () => {
    ...
    setSections(
      (profile.sections as Section[]).filter(
    ...
    setSweepEnabled(
      (bin.maintenance as { enabled?: boolean } | undefined)?.enabled === true,
    );
  }, []);
  useEffect(() => {
    let active = true;
    queueMicrotask(() => {
      if (active)
        void load().catch(() =>
          setMessage("Sense 관리 화면을 불러오지 못했습니다."),
        );
    });
    return () => {
      active = false;
  ...
  return (
    <main className="management-page">
      <header>
        <Link href="/manage">← Corpus 관리</Link>
        <h1>Sense 관리</h1>
        <p>
          일반 섹션과 연결된 Skill을 관리합니다. 민감 섹션은 이 화면에서
          변경하지 않습니다.
        </p>
      </header>
      {!enabled && (
        <p>관리 기능을 준비 중입니다. 아직 변경 작업은 실행할 수 없습니다.</p>
      )}
      <p>
        {sweepEnabled
          ? "자동 정리가 켜져 있습니다."
        ...
        >
          ...
          <div className="management-actions">
            <button
              disabled={
                busy ||
                Boolean(impact.blockers.length) ||
                (impact.action === "purge" && !confirmed)
              }
              onClick={() => void apply()}
            >
              {impact.action === "trash"
