...
type JournalWebMcpRegistry = {
  registerTool(
    tool: JournalWebMcpTool,
...
async function fetchBoard(weekId: string): Promise<BoardResult> {
  return responseResult<BoardResult>(
    await ownerFetch(`/api/journal/board?week=${encodeURIComponent(weekId)}`, {
      cache: 'no-store',
    }),
    '진행 보드를 갱신하지 못했습니다.',
  );
...
export function JournalBoard({
  ...
  const addItem = useCallback(
    async (input: AddItemInput) => {
      ...
      const result = await responseResult<IngestResult>(
        await ownerFetch('/api/journal/items', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...input,
  ...
  const findVisibleItems = useCallback(async (input: FindItemInput) => {
    ...
    try {
      ...
      const result = await responseResult<ItemSearchResult>(
        await ownerFetch(`/api/journal/items?${params}`, { cache: 'no-store' }),
        '항목을 찾지 못했습니다.',
      );
  ...
  const readPeriod = useCallback(async (kind: PeriodKind, anchor: string) => {
    ...
    const result = await responseResult<PeriodResult>(
      await ownerFetch(`/api/journal/period?${params}`, { cache: 'no-store' }),
      '기간 기록을 불러오지 못했습니다.',
    );
    return {
      kind: result.kind,
    ...
    try {
      setDetail(
        await responseResult<ItemDetailResult>(
          await ownerFetch(`/api/journal/items/${encodeURIComponent(itemId)}`, {
            cache: 'no-store',
          }),
          '항목을 불러오지 못했습니다.',
        ),
  ...
  useEffect(() => {
    const context = (document as Document & { modelContext?: JournalWebMcpRegistry }).modelContext;
