(() => {
  ...
  async function ownerFetch(input, init = {}) {
    const identityResponse = await fetch('/api/identity', {
      cache: 'no-store',
      credentials: 'same-origin',
    });
    if (!identityResponse.ok) throw new Error('로그인이 필요합니다.');
    const identity = await identityResponse.json();
    if (typeof identity.csrf !== 'string' || !identity.csrf) {
      throw new Error('로그인 정보를 확인하지 못했습니다.');
    }
    const headers = new Headers(init.headers);
    headers.set('X-Toolkit-CSRF', identity.csrf);
    return fetch(input, {
      ...init,
      headers,
      credentials: 'same-origin',
  ...
  function start() {
    ...
    async function saveEditing({ force = false } = {}) {
      ...
      try {
        ...
        const response = await ownerFetch(`/api/library/issues/${encodeURIComponent(issueId)}`, {
          method: "PATCH",
