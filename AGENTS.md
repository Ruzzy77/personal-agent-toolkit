...
- plugin base version을 바꾼 작업은 소스 커밋과 원격 저장소 반영 뒤 OpenAI workspace plugin, Claude Code와 Claude Desktop을 같은 작업에서 갱신한다. ChatGPT와 Codex에서는 Sense, Corpus, Hypes, Journal과 Library의 제품별 workspace plugin을 한 번만 설치하고, Claude에서는 저장소 marketplace plugin을 사용한다. Document Files와 Design은 Codex와 Claude의 로컬 marketplace plugin으로 유지한다. 새 세션에서 현재 Skill 또는 공개 MCP 도구를 확인한다.
- OpenAI용 Sense·Corpus·Hypes·Journal·Library plugin은 `.app.json`의 제품별 등록 app 하나와 Skill을 함께 배포한다. Codex manifest에 같은 원격 MCP를 직접 선언하거나 `Created by me` 원격 plugin을 별도 설치하지 않는다. Claude plugin과 claude.ai는 상시 원격 MCP endpoint를 직접 연결한다. 각 환경에서 소유자 인증, 현재 작업 목록과 권한을 확인하고, 로컬 서비스를 다시 공개 터널로 연결하거나 원격 MCP와 병행하지 않는다.
