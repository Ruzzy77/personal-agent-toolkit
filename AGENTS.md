...
- `plugins/sense`, `plugins/corpus`와 `plugins/hypes`는 제품 계약과 배포 plugin의 소스, `services/remote-context`는 세 제품이 공유하는 상시 원격 MCP, `apps/sync`는 Finder 자료에 접근하는 로컬 실행 구성으로 다룬다. `gateway`는 이관 검증이 끝날 때까지만 유지하는 기존 터널 경계다. `plugins/journal`은 원격 Journal 연결과 운영 Skill, `services/journal`은 D1 기반 서비스, `sites/journal`은 소유자 전용 화면, `auth`는 공통 소유자 인증 구성으로 다룬다.
