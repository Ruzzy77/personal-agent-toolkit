...
- plugin base version을 바꾼 작업은 소스 커밋과 원격 저장소 반영 뒤 Codex, Claude Code, Claude Desktop과 ChatGPT 웹까지 같은 작업에서 갱신한다. Codex·Claude Code·Claude Desktop에서는 Sense, Corpus, Document Files, Hypes, Journal과 Design을 업데이트하거나 다시 설치하고 새 세션에서 현재 Skill 또는 공개 MCP 도구를 확인한다.
