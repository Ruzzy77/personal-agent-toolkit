...
- Sense, Corpus, Hypes와 Sync의 Python 소스·스크립트·테스트는 루트 `ruff.toml`을 따른다. 변경 뒤 `uvx ruff==0.16.5 format --check plugins/sense plugins/corpus plugins/hypes apps/sync`와 같은 범위의 `ruff check`를 통과시킨다. Document Files는 형식별 파서에 맞춘 `plugins/document-files/pyproject.toml`의 설정과 전체 테스트를 따른다.
