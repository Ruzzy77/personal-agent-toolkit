...
plugin base version은 `products.json`, Claude manifest와 Codex manifest에 같이 반영한다. Codex
manifest의 packaging revision만 바꾸는 경우에는 base version과 service version을 올리지 않는다. 로컬
